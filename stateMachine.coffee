...
class State.Profile extends State
...
class State.Countdown extends State
...
class @StateMachine
	...
	constructor: (states) ->
		@add s for s in states ? []

	add: (s) ->
		throw 'visual must have a name' if not s?.name
		names = s.name.split(',').map (n) -> n?.trim()

		search = _.clone names
		while s not instanceof State and search.length
			n = search.shift()
			ctor = State[Utils.capitalize n]
